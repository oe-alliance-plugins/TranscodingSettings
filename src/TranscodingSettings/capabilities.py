...
class TranscodingCapabilities:
	...
	def __init__(self, proc_root="/proc/stb/encoder", dev_root="/dev", live555_binary=LIVE555_BINARY):
		...
		self.has_live_streaming = self.has_live555_binary and self.has_stream_server_api
		self.is_dream = any(encoder.is_dream for encoder in self.encoders)
	...
	def _publish_box_info(self):
		values = {
			...
			"TranscodingSettingsMultiEncoder": len(self.encoders) > 1,
			"TranscodingSettingsPort8001": True,
			...
			"TranscodingSettingsEncoderDream": self.is_dream
	...
	def port_choices(self):
		choices = [(8001, "8001 - Enigma2 stream server")]
