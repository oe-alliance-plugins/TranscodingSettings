...
def _selectionInt(default, choices):
	choices = [
		(choice[0], choice[1]) if isinstance(choice, tuple) else choice
		for choice in choices
	]
	values = [choice[0] if isinstance(choice, tuple) else choice for choice in choices]
	if default not in values and values:
...
def _bitrate_label(value):
	...
	if value < 0:
		return _("Automatic")
	if value >= 1000000:
		return f"{value / 1000000.0:g} Mbit/s"
...
config.plugins.transcodingsettings.port = _selectionInt(
	8001,
	CAPABILITIES.port_choices(),
)
...
config.plugins.transcodingsettings.advanced = NoSave(ConfigYesNo(default=False))
