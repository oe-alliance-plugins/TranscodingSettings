...
def _pid_matches_daemon(pid):
	...
	try:
		target = readlink(f"/proc/{int(pid)}/exe")
...
def _run_init_action(action):
	if not isfile(LIVE555_INIT_SCRIPT):
		return f"Missing {LIVE555_INIT_SCRIPT}."
	try:
		result = call([LIVE555_INIT_SCRIPT, action], stdout=DEVNULL, stderr=DEVNULL)
	except OSError as error:
		return f"Unable to {action} enigma2-live555: {error}"
	if result:
...
class LiveStreamController:
	...
	def _log(self, message):
		print(f"[TranscodingSettings] {message}")
	...
	def _open_server(self):
		...
		try:
			...
			self._log(f"Unable to access eStreamServer: {error}")
			self.server = None
	...
	def _call(self, method, *args):
		...
		try:
			...
			self._log(f"{method} failed: {error}")
			return None
	...
	def _dbus_error(self, error):
		self._log(f"Live555 D-Bus error: {error}")
	...
	def _start_encoder_service(self, service_ref):
		...
		try:
			...
			self._log(f"Unable to start Dream encoder service: {error}")
		return False
