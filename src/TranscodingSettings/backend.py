...
class TranscodingBackend:
	...
	def _write(self, path, value, errors):
		...
		try:
			...
			errors.append("Unable to write %s: %s" % (path, error))
			return False
	...
	def validate(self):
		errors = []
		self.normalize()
		if str(config.plugins.transcodingsettings.port.value) not in [choice[0] for choice in self.capabilities.port_choices()]:
			errors.append("The selected transcoding port is not available on this receiver.")
		live_enabled = bool(
		...
		if live_enabled:
			if not self.capabilities.has_live555_binary:
				errors.append("HLS/RTSP requires /usr/bin/enigma2-live555.")
			if not self.capabilities.has_stream_server_api:
				errors.append("HLS/RTSP requires the Enigma2 eStreamServer API.")
			if (
				...
				errors.append("HLS and RTSP must use different listening ports.")
			for name, endpoint in (
				...
				if endpoint.enabled.value and endpoint.port.value in (8001, 8002):
					errors.append("%s cannot listen on transcoding port %d." % (name, endpoint.port.value))
		...
		if live_enabled and any(
			...
			if encoder_preference_index(config.plugins.transcodingsettings.encoder.value) < 0:
				errors.append("The selected codec is not supported by every encoder that Auto may allocate for HLS/RTSP.")
			else:
				errors.append("The selected encoder does not support this codec for HLS/RTSP.")
		return errors
